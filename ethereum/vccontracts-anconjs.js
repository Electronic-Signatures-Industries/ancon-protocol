...
const VERIFIABLE_CREDENTIAL_TYPEHASH = web3Utils.soliditySha3( "VerifiableCredential(address issuer,address subject,bytes data,uint256 validFrom,uint256 validTo)" );
...
function getCredentialHash( vc, issuer, claimsVerifierContractAddress ) {
	const payload = web3Abi.encodeParameters(
        // proof + readme props
    );
	...
	const encodeHashCredential = web3Abi.encodeParameters(
		['bytes32', 'address', 'address', 'bytes', 'uint256', 'uint256'],
		[VERIFIABLE_CREDENTIAL_TYPEHASH, issuer, subjectAddress, payload, Math.round( validFrom / 1000 ), Math.round( validTo / 1000 )]
	);
